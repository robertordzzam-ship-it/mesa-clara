...
function PlannerTab({ guests, setGuests, tables, setTables }) {
  ...
  function handleBgUpload(e) {
    const f = e.target.files[0]; if(!f) return;
    setVenueBg(URL.createObjectURL(f)); e.target.value="";
  }
    ...
    <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", position:"relative" }}>
        ...
        <div style={{ background:"var(--fog)", borderBottom:"1px solid var(--sand2)", padding:"8px 16px", display:"flex", gap:10, alignItems:"center", flexShrink:0 }}>
          <input ref={bgFileRef} type="file" accept=".png,.jpg,.jpeg,.pdf" style={{display:"none"}} onChange={handleBgUpload} />
          ...
          {venueBg && <button className="btn" style={{fontSize:12,padding:"6px 10px",background:"#fff2f2",color:"#c0392b",border:"1.5px solid #fecaca"}} onClick={()=>setVenueBg(null)}>✕ Remove map</button>}
          <div style={{flex:1}}/>
          {/* Zoom controls */}
        ...
        >
          ...
          <div style={{ position:"absolute", transformOrigin:"0 0", transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`, width: 900, height: 700 }}>
            ...
            {/* Venue bg */}
            {venueBg && (
              <img src={venueBg} alt="venue" style={{ position:"absolute", top:0, left:0, width:900, height:700, objectFit:"contain", opacity:.45, pointerEvents:"none", borderRadius:8 }} />
            )}
            ...
            {tables.map(t => {
              ...
              return (
                ...
                >
                  {/* Circle — centered inside the padded box */}
                  <div style={{
                    position:"absolute",
                    left: PAD, top: PAD,
...
export default function MesaClara() {
  ...
  const [settings, setSettings] = useState({
    brideName:"Sofia", groomName:"Diego", accent:"#b87c5a", font:"'Fraunces', serif",
